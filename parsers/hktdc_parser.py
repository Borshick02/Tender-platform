...
try:
    from stealth_utils import block_heavy_resources
except ImportError:
    block_heavy_resources = None
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                print("[HKTDC] goto timeout, продолжаем парсинг", file=sys.stderr)
            try:
                page.wait_for_selector(".product-card", timeout=10_000)
            except Exception:
                page.wait_for_timeout(1_500)
            page.wait_for_timeout(min(cfg.parse_delay, 1_500))
            collected = collect_offers(page, cfg)
            print(f"[HKTDC] '{cfg.query}' -> {len(collected)} items")
        except Exception as e:
            print(f"[HKTDC] error: {e}", file=sys.stderr)
        finally:
            context.close()
            browser.close()
        return collected
        ...
        for qi, q in enumerate(queries):
            ...
            try:
                ...
                try:
                    ...
                    print("[HKTDC batch]   goto timeout, пробуем парсить", file=sys.stderr)
                try:
                    page.wait_for_selector(".product-card", timeout=8_000)
                except Exception:
                    page.wait_for_timeout(1_200)
                page.wait_for_timeout(min(cfg.parse_delay, 1_200))
